...
bool USART_PutChar(char c){
	...
	__disable_irq(); //wylaczenie przerwan - sekcja krytyczna

	//wpisanie pojedynczego znaku do bufora kolowego nadawczego
	if(RingBuffer_PutChar(&USART_RingBuffer_Tx, c)){
		
		//wlaczenie przerwan TXE czyli od bufora nadawczego
...
size_t USART_WriteData(const void *data, size_t dataSize){
	...
	//rzutowanie
	const char* dane = (const char* ) data;
	
	//wysylanie znak po znaku do kolowego
	for (; i<dataSize; i++){
		USART_PutChar(*(dane+i));
	}
...
size_t USART_WriteString(const char *string){
	...
	//dlugosc stringa wejsciowego
	size_t string_length = strlen(string);
	
	//przekazanie stringa i jego dlugosci do funkcji wpisujacej
	USART_WriteData(string, string_length);
...
bool USART_GetChar(char *c){
	...
	//Wyciaganie z odbiorczego bufora kolowego do zmiennej char c
...
size_t USART_ReadData(char *data, size_t maxSize){
	...
	//Wczytywanie kolejnych danych z bufora kolowego usart do bufora pod adresem data
	for (; i<maxSize; i++){
...
void USARTx_IRQHandler(void){
	...
	if (__HAL_USART_GET_FLAG(&UartHandle, USART_FLAG_RXNE)) {
		...
		if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_RXNE)) {
			...
			//odczytaj znak z bufora odbiorczego i wrzuc go do odbiorczego bufora kolowego
    ...
    if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_TXE)) {
	    // the TXE interrupt is enabled
			...
			char znak;
			//jezeli odczytanie znaku z bufora kolowego sie powiedzie
			if(RingBuffer_GetChar(&USART_RingBuffer_Tx, &znak)){
				//to wyslij ten odczytany znak
				USARTx->DR = znak;
			}
			else{
			//jezeli odczytanie znaku z bufora sie nie powiedzie - to jest pusty
			//wylacz przerwania TXE
